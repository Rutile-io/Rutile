...
export function getUnsignedTransactionHash(transaction: Transaction): string {
    ...
    return createKeccakHash('keccak256').update(JSON.stringify(dataToHash)).digest('hex');
}
...
export function getTransactionId(transaction: Transaction) {
    ...
    return createKeccakHash('keccak256').update(JSON.stringify(transactionIdData)).digest('hex');
}
